      ...
      <div className="border-b border-slate-100 p-5">
        <div className="flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2.5">
            <CampaignLogo logoUrl={user.campaign?.logoUrl} name={user.campaign?.name || 'Conecta Eleitor'} className="h-9 w-9 shrink-0" />
            <div className="flex flex-col leading-none">
              <p className="text-sm font-bold text-brand-700 tracking-wide">{user.campaign?.publicTitle || user.campaign?.name || 'Conecta Eleitor'}</p>
