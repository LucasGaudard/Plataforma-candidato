...
import { Role, type CampaignContent, type CampaignProposalItem } from '@platform/types';
...
export default function CampaignContentPage() {
  ...
  useEffect(() => {
    api.getCampaignContent().then((data) => setContent(data)).catch((e: Error) => setError(e.message)).finally(() => setLoading(false));
  }, []);
  ...
  async function save(event: FormEvent) {
    ...
    try { setContent(await api.updateCampaignContent(content)); setMessage('Conteúdo salvo com sucesso.'); }
  ...
  return <ProtectedRoute allowedRoles={[Role.ADMIN]}><DashboardLayout title="Conteúdo público" subtitle="Configure a landing page da sua campanha.">
    {loading ? <p>Carregando...</p> : <form onSubmit={save} className="grid gap-6 xl:grid-cols-[1fr_0.8fr]">
      <div className="space-y-5">
        ...
        <button className="rounded-lg bg-brand-600 px-5 py-3 font-semibold text-white">Salvar conteúdo</button>
      </div>
